#!/bin/bash
# Run MedMNIST IPC=1 condensation with training dynamics, reusing existing pretrained models.

set -euo pipefail

PROJECT_DIR="/data/zengqiang/experiments/project_20260419_143900_medmnist_stats"
NCFM_DIR="${PROJECT_DIR}/NCFM"
LOG_DIR="${NCFM_DIR}/ncfm_logs/ipc1_only"
...
find_latest_run_dir() {
    local dataset="$1"
    local base="${PROJECT_DIR}/results/condense/condense/${dataset}/ipc1"
...
run_condense() {
    local dataset="$1"
    local port="$2"
    local log_file="${LOG_DIR}/${dataset}_ipc1.log"

    echo "=== Condense ${dataset} IPC=1 on GPU ${GPU} ==="
    CUDA_VISIBLE_DEVICES="${GPU}" \
    RANK=0 WORLD_SIZE=1 LOCAL_RANK=0 LOCAL_WORLD_SIZE=1 \
    MASTER_ADDR=127.0.0.1 MASTER_PORT="${port}" \
    python condense/condense_script.py \
      --config_path "config/ipc1/${dataset}.yaml" \
      --gpu "${GPU}" \
      --ipc 1 \
...
plot_dynamics_if_exists() {
    ...
    if [ -n "${run_dir}" ] && [ -f "${run_dir}/training_dynamics.json" ]; then
        echo "=== Plot dynamics ${dataset} IPC=1 ==="
