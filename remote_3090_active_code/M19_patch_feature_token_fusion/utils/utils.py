...
from data.transform import transform_imagenet, transform_medmnist
...
def get_plotter(args):
    ...
    if getattr(args, "use_feature_map_token_ncfd", False):
        fmt_layers = getattr(args, "feature_map_token_layers", "last")
        if isinstance(fmt_layers, (list, tuple)):
            fmt_layers = "-".join(str(layer) for layer in fmt_layers)
        base_filename += (
            f"_FMT_lam{getattr(args, 'lambda_feature_map_token_ncfd', 0.0)}"
            f"_tf{getattr(args, 'feature_map_token_num_freqs', 'auto')}"
            f"_lyr{fmt_layers}"
            f"_pos{getattr(args, 'feature_map_token_pos_weight', 0.0)}"
            f"_pool{getattr(args, 'feature_map_token_pool', False)}"
        )
    if getattr(args, "use_local_patch_feature_ncfd", False) and getattr(args, "use_feature_map_token_ncfd", False):
        base_filename += "_M19"
    optimizer_info = {
        "type": args.optimizer,
        "lr": (
