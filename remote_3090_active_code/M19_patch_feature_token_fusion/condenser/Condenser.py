...
class Condenser:
    ...
    def condense(
        ...
        if (
            getattr(args, "use_local_patch_feature_ncfd", False)
            and getattr(args, "use_feature_map_token_ncfd", False)
            and args.rank == 0
        ):
            args.logger(
                "M19 fusion enabled: "
                f"L_total = L_global + {getattr(args, 'lambda_local_patch_ncfd', 0.0)} * L_patch + "
                f"{getattr(args, 'lambda_feature_map_token_ncfd', 0.0)} * L_token"
            )
        ...
        for it in range(args.niter):
            ...
            if it % args.it_log == 0 and args.rank == 0:
                ...
                if (
                    getattr(args, "use_local_patch_feature_ncfd", False)
                    and getattr(args, "use_feature_map_token_ncfd", False)
                    and hasattr(args, "_lp_last_total_loss")
                    and hasattr(args, "_fmt_last_total_loss")
                ):
                    m19_msg = (
                        f" m19-global-loss: {getattr(args, '_fmt_last_global_loss', 0.0):.4f} "
                        f"m19-patch-loss: {getattr(args, '_lp_last_local_loss', 0.0):.4f} "
                        f"m19-token-loss: {getattr(args, '_fmt_last_token_loss', 0.0):.4f} "
                        f"m19-total-loss: {getattr(args, '_fmt_last_total_loss', 0.0):.4f}"
                    )
                if args.iter_calib > 0:
                    args.logger(
                        f"\n{get_time()} (Iter {it:3d}) "
                        ...
                        f"{dam_msg} {lp_msg} {dgsa_msg} {fmt_msg} {m19_msg} "
