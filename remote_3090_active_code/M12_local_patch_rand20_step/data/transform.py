...
def transform_medmnist(
    dataset, size=28, augment=False, from_tensor=False, normalize=True
):
    size = 28 if size in (None, 0, -1) else int(size)
    if not augment:
        aug = []
    else:
        aug = [transforms.RandomCrop(size, padding=4)]
        print(f"Dataset with basic {dataset} augmentation")
