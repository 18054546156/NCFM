...
class Condenser:
    ...
    def condense(
        ...
        if getattr(args, "use_ssim_regularization", False) and args.rank == 0:
            args.logger(
                "SSIM regularization enabled: "
                f"weight={getattr(args, 'ssim_weight', 0.1)}, "
                f"grids={getattr(args, 'ssim_grids', [1])}, "
                f"kernel={getattr(args, 'ssim_kernel_size', 7)}, "
                f"pairing={getattr(args, 'ssim_pairing', 'prototype')}"
            )
        ...
        for it in range(args.niter):
            ...
            if it % args.it_log == 0 and args.rank == 0:
                ...
                ssim_msg = ""
                if getattr(args, "use_ssim_regularization", False) and hasattr(args, "_ssim_last_loss"):
                    ssim_msg = (
                        f" ssim-global-loss: {getattr(args, '_ssim_last_global_loss', 0.0):.4f} "
                        f"ssim-loss: {getattr(args, '_ssim_last_loss', 0.0):.4f} "
                        f"ssim-total-loss: {getattr(args, '_ssim_last_total_loss', 0.0):.4f} "
                        f"ssim-components: {getattr(args, '_ssim_last_components', {})}"
                    )
                if args.iter_calib > 0:
                    args.logger(
                        f"\n{get_time()} (Iter {it:3d}) "
