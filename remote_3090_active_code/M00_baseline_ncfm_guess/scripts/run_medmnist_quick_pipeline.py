...
def run_condense_eval_cam(args, repo_dir, dataset, ipc, group_name, niter):
    ...
    append(
        ...
        f"| {datetime.now().isoformat(timespec='seconds')} | {dataset} | {ipc} | {group_name} | done | {accuracy} | `{condensed_path}` | `{checkpoint_path}` | `{cam_dir / 'summary.csv'}` | quick protocol |\n",
    )
    append(
        args.exp_root / "CAM_RESULTS.md",
        f"| {dataset} | ipc{ipc}_{group_name} | `{checkpoint_path}` | `{cam_dir / 'summary.csv'}` | `{cam_dir / 'overlays'}` | quick protocol |\n",
    )
    return metrics
...
def run_real_cam(args, repo_dir, dataset):
    ...
    append(
        args.exp_root / "CAM_RESULTS.md",
        f"| {dataset} | real_train | `{checkpoint_path}` | `{cam_dir / 'summary.csv'}` | `{cam_dir / 'overlays'}` | quick pretrain |\n",
    )


def main():
    parser = argparse.ArgumentParser(description="Run quick MedMNIST A/B/C + CAM pipeline.")
    ...
    patch_text = """
## Applied quick-pipeline patches

- `models/convnet.py`: keep the 28->32 feature-size adjustment only for grayscale 28x28 inputs, fixing RGB 28x28 MedMNIST ConvNet shape mismatches.
- `condense/condense_script.py`: infer feature dimension with a dummy forward pass and create `SampleNet(feature_dim=inferred_dim, t_batchsize=args.num_freqs)`.
- `condenser/evaluate.py`: save the best evaluator checkpoint for CAM; initialize best accuracy to -1 so a checkpoint is always saved at first evaluation.
- `evaluation/evaluation_script.py`: add `--eval_checkpoint_path` and augmentation flags.
- `utils/init_script.py`: always save the final condensed data at `niter`, which matters for short smoke runs.
- `evaluation/cam_utils.py` and `evaluation/run_cam.py`: standalone Grad-CAM generation.
- `scripts/medmnist_sanity.py`, `scripts/model_sanity.py`, `scripts/run_medmnist_quick_pipeline.py`: reproducibility helpers.
"""
    append(args.exp_root / "PATCH_NOTES.md", patch_text)
    ...
    for dataset, ipc, niter in phase_plan:
        for group_name in GROUPS:
            all_metrics.append(run_condense_eval_cam(args, repo_dir, dataset, ipc, group_name, niter))
