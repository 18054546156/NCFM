...
def build_groups():
    ...
    for name, weight, grids in ssim_specs:
        groups[name] = {
            "sampling_net": True,
            "num_freqs": 512,
            "iter_calib": 0,
...
def assert_assets(exp_root):
    missing = []
    data_path = exp_root / "data" / "medmnist" / "bloodmnist.npz"
    pretrain_dir = exp_root / "checkpoints" / "pretrain" / "bloodmnist"
    if not data_path.exists():
        missing.append(str(data_path))
    for idx in range(20):
        trained = pretrain_dir / f"premodel{idx}_trained.pth.tar"
        init = pretrain_dir / f"premodel{idx}_init.pth.tar"
        if not trained.exists():
            missing.append(str(trained))
        if not init.exists():
            missing.append(str(init))
    if missing:
        raise FileNotFoundError("Required BloodMNIST assets are missing:\n" + "\n".join(missing))


def save_group_plan(exp_root, groups, family):
    report_dir = exp_root / "reports"
    report_dir.mkdir(parents=True, exist_ok=True)
    csv_path = report_dir / "bloodmnist_seed0_group_plan.csv"
    md_path = report_dir / "bloodmnist_seed0_group_plan.md"
...
def save_progress(exp_root, selected, completed, failed):
    status_path = exp_root / "RUN_STATUS.md"
    lines = [
        "# BloodMNIST Seed0 Sweep Status",
        "",
...
def main():
    parser = argparse.ArgumentParser(description="Run BloodMNIST seed0 28-group NCFM method sweep.")
    ...
    parser.add_argument("--summary_prefix", default="bloodmnist_seed0_worker_summary")
    ...
    if args.ipc != IPC:
        raise ValueError("This runner is fixed to BloodMNIST IPC=10 for the 20260530 sweep.")
    ...
    assert_assets(args.exp_root)
    save_group_plan(args.exp_root, groups, family)
    ...
    for group_name in selected:
        print(f"RUN dataset={DATASET} group={group_name} family={family[group_name]} gpu={args.gpu}", flush=True)
        try:
            metrics = pipeline.run_condense_eval_cam(args, repo_dir, DATASET, IPC, group_name)
            ...
            save_progress(args.exp_root, selected, completed, failed)
            raise
        save_progress(args.exp_root, selected, completed, failed)
