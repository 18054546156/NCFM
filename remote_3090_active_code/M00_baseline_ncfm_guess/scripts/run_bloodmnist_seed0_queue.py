...
from run_bloodmnist_seed0_sweep import DATASET, IPC, build_groups, load_pipeline
...
def main():
    parser = argparse.ArgumentParser(description="Lock-based BloodMNIST seed0 queue worker.")
    ...
    parser.add_argument("--summary_prefix", default="bloodmnist_seed0_queue")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()

    if args.ipc != IPC:
        raise ValueError("This queue is fixed to BloodMNIST IPC=10.")
    ...
    for group in groups:
        if group_done(args.exp_root, group) and not args.force:
            continue
        ...
        print(f"CLAIM group={group} family={family[group]} gpu={args.gpu} worker={args.worker_id}", flush=True)
        try:
            ...
            print(f"DONE group={group} acc={metrics.get('acc_percent')}", flush=True)
            ...
            print(f"FAILED group={group} error={exc!r}", flush=True)
