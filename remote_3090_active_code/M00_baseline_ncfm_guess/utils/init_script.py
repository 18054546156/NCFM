...
def set_experiment_name_and_save_Dir(
    ...
    exp_tag = str(exp_tag or "").strip()
    if exp_tag:
        exp_tag = "".join(c if c.isalnum() or c in "._-" else "_" for c in exp_tag)
        exp_tag = f"_{exp_tag}"
    ...
    if run_mode == "Condense":
        ...
        exp_name = f"./condense/{dataset}/ipc{ipc}/{optimizer}_lr_img_{lr_img:.4f}_numr_reqs{num_freqs}_factor{factor}{exp_tag}_{timestamp}"
        if load_path:
        ...
        assert ipc > 0, "IPC must be greater than 0"
        exp_name = (
            f"./evaluate/{dataset}/ipc{ipc}/_lr{lr:.4f}__factor{factor}{exp_tag}_{timestamp}"
        )
        save_dir = os.path.join(save_dir, exp_name)
